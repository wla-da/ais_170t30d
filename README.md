...
- [Приемник AIS на базе модуля E32 170T30D от EByte](#приемник-ais-на-базе-модуля-e32-170t30d-от-ebyte)
  - [Задачи](#задачи)
  - [Софт](#софт)
  - [Железо](#железо)
  - [Выводы (спойлер)](#выводы-спойлер)
  - [Определяемся с архитектурой](#определяемся-с-архитектурой)
    - [Схема (частично) модуля E32 170T30D](#схема-частично-модуля-e32-170t30d)
      - [С чем предстоит разобраться?](#с-чем-предстоит-разобраться)
      - [Фото платы крупным планом](#фото-платы-крупным-планом)
      - [Схема подключения SX1278 и MCU на плате E32 170T30D](#схема-подключения-sx1278-и-mcu-на-плате-e32-170t30d)
  - [Базовый алгоритм работы радиочипа](#базовый-алгоритм-работы-радиочипа)
  - [Сборка патченной версии OpenOCD от Spritetm для прошивки HC32L110](#сборка-патченной-версии-openocd-от-spritetm-для-прошивки-hc32l110)
  - [Подключаем плату E32 170T30D к ST-link v2](#подключаем-плату-e32-170t30d-к-st-link-v2)
  - [Подключаем плату E32 170T30D к USB-UART конвертору для прошивки через ISP](#подключаем-плату-e32-170t30d-к-usb-uart-конвертору-для-прошивки-через-isp)
  - [Способы разблокировки HC32L110 с зашифрованной прошивкой](#способы-разблокировки-hc32l110-с-зашифрованной-прошивкой)
    - [1) Отправка спецсимволов через UART в MCU](#1-отправка-спецсимволов-через-uart-в-mcu)
    - [2) Использование HcUnlock.exe:](#2-использование-hcunlockexe)
    - [3) Стандартный ISP прошивальщик (китайский):](#3-стандартный-isp-прошивальщик-китайский)
  - [Подготовка тулчейна для разработки под HC32L110](#подготовка-тулчейна-для-разработки-под-hc32l110)
    - [Компилятор, линковщик, полезные функции](#компилятор-линковщик-полезные-функции)
  - [Исследование пакетного режима SX1278](#исследование-пакетного-режима-sx1278)
    - [Эксперименты по включению автоподстройки частоты (AFC)](#эксперименты-по-включению-автоподстройки-частоты-afc)
  - [Исследование непрерывного режима (continuous mode) SX1278](#исследование-непрерывного-режима-continuous-mode-sx1278)
  - [TODO Идеи улучшения](#todo-идеи-улучшения)
  - [Что дальше?](#что-дальше)
  - [Ссылки](#ссылки)

## Задачи
1. Определиться с архитектурой приемника: один MCU+USB мост, два MCU и тп
2. Разобраться с программированием и прошивкой MCU HC32L110
...
**Цель**: Создать приёмник AIS на базе готового модуля E32 170T30D (SX1278 + HC32L110) с минимальными доработками железа.
...
5. AFC не отрабатывает корректно без детекта преамбулы — частотная ошибка компенсируется частично, не стабильно работает бит-синхронизатор: пропуски пакетов и мусор на выходе
6. Кварц 26 MHz (измерено 26.000387 MHz), требуется TCXO для стабильного приёма AIS (дрейф < 500 Гц)
Рекомендация: Для полноценного AIS приёмника SX1278 не оптимален. Рассмотреть AX5043, CMX910 или специализированные AIS чипы. Если использовать SX1278 — обязательна перепайка DIO пинов + TCXO + внешняя фильтрация.
...
4. Есть RF ключ HWS421 для коммутации Rx/Tx с двумя входами управления, подключенными к P34 и P33 MCU (пины те же самые, что управляют вкл/выкл стабилизаторами напряжений)
...
4) включаем автоподстройку частоты с автоматическим сбросом AfcAutoClearOn = 1, т.к. преамбула задана - AFC должна работать корректно 
...
Далее, если MCU уже разблокирован и работает SWD, для прошивки можно использовать PyOCD. 

Настроил в VS Code удобные кнопки с отображением на статус-баре для сборки, прошивки, прошивки с автоматическим монитором UART и очистки проекта с помощью расширения (плагина) VS Code Action Buttons. Установка через маркетплейс или командой:
...
# посмотреть занимаемый размер секций
...
Включен бит-синхронизатор (в аппаратном режиме это делается принудительно).
...
По итогу получил сравнительное неплохие результаты в части стабильного распознавания пакетов при чувствительности около -100 -105 дБм без NRZI кодировки.

Далее решил вернуть на генераторе NRZI кодирование, а в радиочипе SX1278 изменил настройки:
...
| RegRxConfig   | 0x0D  | AgcAutoOn (3)      | Включение AGC, отключает настройки усиления LNA, регистр RegLna (0x0C)   
...
В случае приема AIS преамбула находится закодирована в NRZI и может быть представлена в эфире четырьмя различными последовательностями:  0x33, 0x66, 0x99 или 0xCC. Например, так работает AIS процессор на базе CMX910.

Таким образом, хотя прием валидных пакетов AIS и осуществляется, но стабильность приема даже в тестовых условиях не очень хорошая. Сюда же накладываются ограничения из-за множества различных комбинаций преамбулы, а чип может обнаруживать только одно синхрослово. Да, можно делать периодическое сканирование эфира с разными значениями синхрослова или искать параллельно полностью на MCU.

Остаются вопросы нестабильной работы механизма AFC (особенно актуальны для узкополосного AIS сигнала) и, как следствие, сниженной чувствительности приемника и пропуска пакетов. Поэтому реализовывать полноценный AIS приемник на данном чипе считаю нецелесообразным.


## Исследование непрерывного режима (continuous mode) SX1278

Для непрерывного режима необходимо подключить DIO2 SX1278 к одному с свободных портов MCU, требуется припаять провод (на плате DIO пины не разведены).

Чтобы попробовать получить улучшения по сравнению с пакетным режимом необходимо отключить бит-синхронизатор и проводить восстановление тактовой частоты ([clock recovery](https://www.dwdm.ru/wiki/100)) на внешнем MCU, например, читая значение пина DIO2 с оверсэмплингом x16...x64 и даже выше. Чип SX1278 не предоставляет аналоговый после демодулятора или IQ сигнал для восстановления фазы и использует для бинаризации, по всей видимости, простой компаратор. Это сразу дает несколько единиц дБ не восстановимых потерь в чувствительности приемника.

"Переговорив" подробно с несколькими разными LLM  в части оценки улучшения приема в непрерывном режиме получил оценки, что максимальная чувствительность при приемлемом BER будет не лучше -100..-105 дБм. При этом придется реализовать достаточно серьезную программную часть для обработки входящего битового потока (clock recovery, обнаружение преамбулы в разных вариациях, поиск HDLC-флагов и тп).

Встроенный на плату MCU HC32L110U4A явно не потянет в реальном режиме времени полноценный clock recovery. Потребуется прямое подключение DIO2 SX1278 к более мощному MCU, например, ESP32. Так как "проброс" даже "сырых" бит через HC32L110U4A даст дополнительный джиттер и увеличит ошибки.

Усилия на реализацию, на мой взгляд, не стоят того, т.к. полученная чувствительность будет на ~20 дБ хуже достижимой на практике.
...
3. Методы ЦОС: восстановление тактирования, автоподстройка частоты (компенсация)
