...
И всё было бы хорошо, если бы SX1278 поддерживал установку кастомной преамбулы. Но чип умеет обнаруживать преамбулу только с фиксированной последовательностью байт: либо 0x55 либо 0хAA (конкретное значение выбирается битом в регистре). А в нашем случае преамбула в эфире существует в виде NRZI кодированной последовательности байт 0xCC 0xCC 0xCC или 0x33 0x33 0x33 (в зависимости от начального бита). Как следствие, не срабатывает детектор преамбулы, не работает (корректно) AFC по преамбуле (так зависит от работы FEI - Frequency Error Indicator), не работает (корректно) бит-синхронизатор:
...
6. Чувствительность выше ~120-123 дБм на практике смысла не умеет: предел: -174 + 10Log(BW) = -174 + 41 = ~ -133 дБм + SNR(10-12 дБ)
